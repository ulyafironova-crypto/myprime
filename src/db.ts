...
export async function bootstrap(){const d=await db;if(!(await d.get('meta','notes-imported'))){const tx=d.transaction(['workouts','meta'],'readwrite');initialRuns.forEach(workout=>tx.objectStore('workouts').put(workout));tx.objectStore('meta').put(true,'notes-imported');await tx.done}}
